...
void HitmapCollection::Fill(const eudaq::StandardEvent &ev, int evNumber) {
  ...
  for (int plane = 0; plane < ev.NumPlanes(); plane++) {
    ...
    if (Plane.Sensor().find("HexaBoard")!=std::string::npos) {std::cout << "I found hexaboard plane" << std::endl; continue;}
    ...
    SimpleStandardPlane simpPlane(Plane.Sensor(),Plane.ID(),Plane.XSize(),Plane.YSize(),&_mon->mon_configdata);
