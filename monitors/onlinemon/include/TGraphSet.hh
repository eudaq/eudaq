...
public:
  ...
  void SetLimitsFromSlider(const int x1, const int x2, const int y1,
                           const int y2);
  void GetXLimits(double &min, double &max);
  void GetYLimits(double &min, double &max);
  void SetZoom(const int x1, const int x2, const int y1, const int y2) {
    _xLimitMin = x1 + _starttime;
    _xLimitMax = x2 + _starttime;
    _yLimitMin = y1;
    _yLimitMax = y2;
  }
