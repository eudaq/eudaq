...
//!There is only one histogram per TreeItem, but the Draw-Method will draw also
//all histograms in the children of the TreeItem using the NextSibling-Method,
//typically an object like this contains an histogram or has at least one child
class HistoItem {
protected:
...
public:
  ...
  HistoItem(TGListTreeItem *tree, std::string str) //$$ change removed const
      : _treeItem(tree),
        ...
        _histoOptions(""),
        _histoLogOption(0),
        _fCanvas(0) {} //_histoLogOptions  --> _histoLogOption
  //!Nothing to do for the destructor
  virtual ~HistoItem(){};

  //!Add histograms to the TreeItem
  void setHisto(const TH1 *histo, const std::string opt = "",
                const unsigned int logOpt = 0);
  //!If the Draw-Methods draws more than one histo the parent treeitems needs to
  //know which of them to show in an overview
  void setOverviewHisto(TH1 *histo) {
    _overviewHisto = histo;
  } //$$ change removed const
...
//! This is the container-class for all TreeItem and histogram relationships.
//The drawing mechanism should work from HistoItemClass itself, this class
//should help to convert the different objects instead of using maps
class HistoTreeContainer {
protected:
...
public:
  ...
  //! Getter for the HistoItems - overloaded argument is the TreeItem - use this
  //to call Draw() from the TreeBrowser e.g.
  HistoItem const getHistoItem(TGListTreeItem *item) { return _treeItem[item]; }
  //! Getter for the HistoItems - overloaded argument is the histogrampointer -
  //use this to call Draw() from the Clicking for Histograms e.g.
