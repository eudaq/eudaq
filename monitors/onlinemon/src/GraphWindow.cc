...
GraphWindow::GraphWindow(const TGWindow *p, UInt_t w, UInt_t h)
    : TGMainFrame(p, w, h), TimeSliderMin(-1), TimeSliderMax(1),
      GraphSliderMin(-1), GraphSliderMax(1) {
  cout << "Creating new GraphWindow" << endl;

  Hfrm_Graph = new TGHorizontalFrame(this);
  GraphSlider = new TGDoubleVSlider(Hfrm_Graph, 1, 1, -1, kVerticalFrame,
                                    GetDefaultFrameBackground(), kTRUE, kFALSE);
  GraphSlider->Connect("PositionChanged()", "GraphWindow", this, "DoSlider()");
  GraphSlider->SetRange(-1, 1);
  GraphSlider->SetPosition(-1, 1);
  Hfrm_Graph->AddFrame(
      GraphSlider, new TGLayoutHints(kLHintsLeft | kLHintsExpandY, 2, 2, 2, 2));

  ECvs_Graph = new TRootEmbeddedCanvas("Values", Hfrm_Graph);
  (TCanvas *)ECvs_Graph->Connect("ProcessedEvent(Int_t,Int_t,Int_t,TObject*)",
                                 "GraphWindow", this,
                                 "Selected(Int_t,Int_t,Int_t,TObject*)");
  Hfrm_Graph->AddFrame(
      ECvs_Graph,
      new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 2, 5, 5, 2));
  ...
  AddFrame(Hfrm_Graph,
           new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 5, 5, 5, 2));
...
void GraphWindow::Update(bool firsttime) {
  ...
  TimeSlider->SetRange((Float_t)min,(Float_t)max);
  ...
  TimeSlider->SetPosition((Float_t)(oldminTime - TimeSliderMin + min),
                          (Float_t)(oldmaxTime - TimeSliderMax + max));
  TimeSliderMin = min;
  TimeSliderMax = max;

  set->GetYLimits(min, max);
  GraphSlider->GetPosition(oldminGraph, oldmaxGraph);
  GraphSlider->SetRange((Float_t)min, (Float_t)max);

  // here it is the same //somethig is strange in this line.... hmn
  GraphSlider->SetPosition((Float_t)(oldminGraph - GraphSliderMin + min),
                           (Float_t)(oldmaxGraph - GraphSliderMax + max));
  std::cout << "Pos: " << oldminGraph << " / " << GraphSliderMin << " / " << min
            << std::endl;
  // std::cout << "Setting to: " << oldminGraph-GraphSliderMin+min << " / " <<
  // oldmaxGraph - GraphSliderMax+max << std::endl;
  GraphSliderMin = min;
  GraphSliderMax = max;
  TimeSlider->GetPosition(oldminTime, oldmaxTime);
  GraphSlider->GetPosition(oldminGraph, oldmaxGraph);
  ...
  set->SetZoom((int)oldminTime, (int)oldmaxTime, (int)oldminGraph, (int)oldmaxGraph);
...
void GraphWindow::DoSlider() {
  ...
  set->SetLimitsFromSlider((int)x1, (int)x2, (int)y1, (int)y2);
