...
void HitmapCollection::registerPlane(const SimpleStandardPlane &p) {
  ...
  if (_mon != NULL) {
    ...
    _mon->getOnlineMon()->registerHisto(
        ...
        "COLZ", 0);
    if ((p.is_APIX) || (p.is_USBPIX) || (p.is_USBPIXI4) || (p.is_RD53A) || (p.is_RD53B) || (p.is_ITKPIXV2) || (p.is_RD53BQUAD) || (p.is_ITKPIXV2QUAD) || (p.is_RD53BTRIPLETLINEAR) || (p.is_ITKPIXV2TRIPLETLINEAR)) {
