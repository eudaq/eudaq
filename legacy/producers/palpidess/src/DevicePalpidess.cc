...
/*****************************************************************************
 * Device pALPIDEss receives the event from via UDP/IP                       *
 * In this class implemented:                                                *
 * - The methods for create a connection with server UDP/IP                  *
 *   (create_server_udp, close_server, get_SD)                               *
 * - The methods for configuring and getting events                          *
 *   (Configure, ConfigureDAQ, SingleEvent)                                  *
 *                                                                           *
 *****************************************************************************/
...
// Costructor
//-----------------------------------------------------------------------------
DevicePalpidess::DevicePalpidess() : data(-1), sc(-1), cmdRdy(system(NULL)) {}
// Costructor
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int DevicePalpidess::create_server_udp(
    int port /* = PORT */) { // create udp serves for fecs
  int server = -1;
  ...
  if (bind(server, (struct sockaddr *)&srvAddr, sizeof(srvAddr)) <
      0) // bind adress
...
void DevicePalpidess::close_server_udp(int sd) { close(sd); }
...
bool DevicePalpidess::ConfigureDAQ() {
  if (cmdRdy) {
    system("${PALPIDESS_SCRIPTS}/ConfigureDAQ.sh");
  } else
    return false;
  cmdRdy = false;
...
bool DevicePalpidess::GetSingleEvent() {
  if (sc >= 0) {
    const unsigned char re[20] = {
        0x80, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xAA, 0xBB, 0xFF,
        0xFF, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x04}; // rising edge
                                                               // for the event
                                                               // request
