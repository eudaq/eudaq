...
The executable programs mainly split up in two different categories: Processes, which are used for the data acquisition and communicating with the Run Control (DAQ), and utilities, which are used after the data taking in order to access the data files (Tools).
Only a few executables belong to a third category, which includes tools for development or testing before data taking (Test).
...
\begin{tabular}{ l | l | l | p{6cm}}
  ...
  \hline			
  DAQ & Run Control & \texttt{euRun.exe} (GUI) & bla \\
  \hline  
\end{tabular}
