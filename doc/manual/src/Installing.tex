...
%\footnote{To install the binaries and the library outside the source tree, you need to set the \texttt{INSTALL\_PREFIX} option, e.g.
%\begin{listing}[mybash]
%cmake -D INSTALL\_PREFIX=/usr/local ..
%\end{listing}
...
%\begin{listing}[mybash]
%cd eudaq/build/
%sudo xargs rm < install\_manifest.txt
%\end{listing}
%}
