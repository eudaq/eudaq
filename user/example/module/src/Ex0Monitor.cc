...
public:
  explicit Ex0Monitor(const std::string &name, const std::string &runcontrol);
  ...
  static constexpr uint32_t m_id_factory = eudaq::cstr2hash("Ex0Monitor");
...
namespace {
  auto dummy0 =
      eudaq::Factory<eudaq::Monitor>::Register<Ex0Monitor, const std::string &,
                                               const std::string &>(
          Ex0Monitor::m_id_factory);
}
