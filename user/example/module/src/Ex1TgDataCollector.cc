...
class Ex1TgDataCollector:public eudaq::DataCollector{
public:
  Ex1TgDataCollector(const std::string &name,
		   const std::string &rc);
  ...
  static const uint32_t m_id_factory = eudaq::cstr2hash("Ex1TgDataCollector");
private:
  std::mutex m_mtx_map;
...
namespace{
  auto dummy0 = eudaq::Factory<eudaq::DataCollector>::
    Register<Ex1TgDataCollector, const std::string&, const std::string&>
    (Ex1TgDataCollector::m_id_factory);
}

Ex1TgDataCollector::Ex1TgDataCollector(const std::string &name,
				       const std::string &rc):
  DataCollector(name, rc){
}

void Ex1TgDataCollector::DoConnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_conn_evque[idx].clear();
  m_conn_inactive.erase(idx);
}

void Ex1TgDataCollector::DoDisconnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
...
void Ex1TgDataCollector::DoConfigure(){
  m_noprint = 0;
  auto conf = GetConfiguration();
  if(conf){
    conf->Print();
    m_noprint = conf->Get("EX1_DISABLE_PRINT", 0);
  }
}

void Ex1TgDataCollector::DoReset(){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_noprint = 0;
  m_conn_evque.clear();
  m_conn_inactive.clear();
...
void Ex1TgDataCollector::DoReceive(eudaq::ConnectionSPC idx, eudaq::EventSP evsp){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  ...
  auto ev_sync = eudaq::Event::MakeUnique("Ex1Tg");
