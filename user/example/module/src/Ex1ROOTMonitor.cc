...
// let there be a user-defined Ex1EventDataFormat, containing e.g. three
// double-precision attributes get'ters:
//   double GetQuantityX(), double GetQuantityY(), and double GetQuantityZ()
struct Ex1EventDataFormat {
  Ex1EventDataFormat(const eudaq::Event&) {}
...
class Ex1ROOTMonitor : public eudaq::ROOTMonitor {
public:
  Ex1ROOTMonitor(const std::string& name, const std::string& runcontrol):
    eudaq::ROOTMonitor(name, "Ex1 ROOT monitor", runcontrol){}

  void AtConfiguration() override;
  void AtEventReception(eudaq::EventSP ev) override;

  static const uint32_t m_id_factory = eudaq::cstr2hash("Ex1ROOTMonitor");
...
namespace{
  auto mon_rootmon = eudaq::Factory<eudaq::Monitor>::
    Register<Ex1ROOTMonitor, const std::string&, const std::string&>(Ex1ROOTMonitor::m_id_factory);
}

void Ex1ROOTMonitor::AtConfiguration(){
...
void Ex1ROOTMonitor::AtEventReception(eudaq::EventSP ev){
  auto event = std::make_shared<Ex1EventDataFormat>(*ev);
