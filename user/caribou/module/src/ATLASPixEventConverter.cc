...
namespace {
  auto dummy0 = eudaq::Factory<eudaq::StdEventConverter>::Register<
      ...
      ATLASPixEvent2StdEventConverter::m_id_factory);
}
...
bool ATLASPixEvent2StdEventConverter::Converting(
  ...
  if (datain & 0x80000000) {
    ...
    if (t0_seen_ == 0) {
      ...
      throw DataInvalid("Detected 2nd T0 signal in event: " +
                        std::to_string(ev->GetEventNumber()) + " (ts signal)");
    }
    // Structure: {1'b1, column_addr[5:0], row_addr[8:0], rise_timestamp[9:0],
    // fall_timestamp[5:0]} Extract pixel data
    long ts2 = gray_decode((datain) & 0x003F);
    // long ts2 = gray_decode((datain>>6)&0x003F);
    // TS1 counter is by default half speed of TS2. By multiplying with 2 we
    // make it equal.
    long ts1 = (gray_decode((datain >> 6) & 0x03FF)) << 1;
    // long ts1 = (gray_decode(((datain << 4) & 0x3F0) | ((datain >> 12) &
    // 0xF)))<<1;
    ...
    if (ts_diff > 0) {
      // Hit probably came before readout started and meanwhile an OVF of TS1
      // happened
      if (ts_diff > 0x01FF) {
        ts_diff -= 0x0800;
      }
    ...
    // calculate ToT only when pixel is good for storing (division is time
    // consuming)
    int tot = static_cast<int>(
        ts2 -
        ((hit_ts % static_cast<long long>(64 * clkdivend2)) / clkdivend2));
    if (tot < 0) {
      tot += 64;
    }

    LOG(DEBUG) << "HIT: TS1: " << ts1 << "\t0x" << std::hex << ts1
               << "\tTS2: " << ts2 << "\t0x" << std::hex << ts2
    ...
    if (message_type == 0b01000000) {
      ...
      long long ts_diff = static_cast<long long>(atp_ts) -
                          static_cast<long long>(fpga_ts_ & 0x1FFFF);
      ...
      readout_ts_ = static_cast<unsigned long long>(
          static_cast<long long>(fpga_ts_) + ts_diff);
      LOG(DEBUG) << "RO_ts " << std::hex << readout_ts_ << " atp_ts " << atp_ts
                 << std::dec;
      ...
      fpga_ts1_ = ((static_cast<unsigned long long>(datain) << 48) &
                   0xFFFF000000000000);
      new_ts1_ = true;
    } else if (message_type == 0b00100000) {

      // Timestamp from FPGA [47:24] (3/4)
      uint64_t fpga_tsx = ((static_cast<unsigned long long>(datain) << 24) &
                           0x0000FFFFFF000000);
      ...
      // BUSY was asserted due to FIFO_FULL + 24 LSBs of FPGA timestamp when it
      // happened
      ...
      if (message_type & 0b11110010) {
        ...
        // Buffer for chip data overflow (data that came after this word were
        // lost)
        ...
        // SERDES lock lost (data might be nonsense, including up to 2 previous
        // messages)
