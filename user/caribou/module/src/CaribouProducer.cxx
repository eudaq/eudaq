...
private:
  ...
  device::DeviceManager *manager_;
  device::Device *device_{nullptr};
  device::Device *secondary_device_{nullptr};
  std::string name_;
...
CaribouProducer::CaribouProducer(const std::string name,
  ...
  manager_ = new device::DeviceManager();
}
...
void CaribouProducer::DoInitialise() {
  ...
  try {
    log::LogLevel log_level = log::Log::getLevelFromString(level);
    log::Log::setReportingLevel(log_level);
  ...
  level_ = log::Log::getReportingLevel();
...
void CaribouProducer::RunLoop() {
  ...
  log::Log::setReportingLevel(level_);
  ...
  while (!m_exit_of_run) {
    ...
    } catch (device::DataTakingError &e) {
