...
bool CLICpix2Event2StdEventConverter::Converting(
  ...
  static auto matrix_config = [counting, longcnt]() {
    std::map<std::pair<uint8_t, uint8_t>, dut::pixelConfig> matrix;
    for (uint8_t x = 0; x < 128; x++) {
      for (uint8_t y = 0; y < 128; y++) {
        // FIXME hard-coded matrix configuration for CLICpix2 - needs to be read
        // from a configuration!
        matrix[std::make_pair(y, x)] =
            dut::pixelConfig(true, 3, counting, false, longcnt);
      }
    }
    return matrix;
  };
  ...
  static dut::clicpix2_frameDecoder decoder(comp, sp_comp, matrix);
  // No event
  if (!ev) {
    return false;
  }
  ...
  for (const auto &px : data) {
    auto cp2_pixel = dynamic_cast<dut::pixelReadout *>(px.second.get());
