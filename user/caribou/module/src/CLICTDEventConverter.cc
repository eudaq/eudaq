...
namespace {
  auto dummy0 = eudaq::Factory<eudaq::StdEventConverter>::Register<
      ...
      CLICTDEvent2StdEventConverter::m_id_factory);
}
...
bool CLICTDEvent2StdEventConverter::Converting(
  ...
  // Integer to allow skipping pixels with certain ToT values directly when
  // decoding
  auto discard_tot_below = conf->Get("discard_tot_below", -1);
  auto discard_toa_below = conf->Get("discard_toa_below", -1);

  static dut::CLICTDFrameDecoder decoder(longcnt);
  // No event
  if (!ev) {
    return false;
  }
  ...
  utils::pearyRawData rawdata;

  // Retrieve data from event
  if (ev->NumBlocks() == 1) {
    // New data format - timestamps and pixel data are combined in one data
    // block

    // Block 0 contains all data, split it into timestamps and pixel data,
    // returned as std::vector<uint8_t>
  ...
  if (ev->GetEventNumber() == 1) {
    for (auto &timestamp : timestamps) {
      EUDAQ_INFO("TS " + eudaq::to_bit_string((timestamp >> 48), 16, true) +
                 " " + std::to_string(timestamp & 0xffffffffffff));
    }
  }
  ...
  for (auto &timestamp : timestamps) {
    ...
    if (legacy_format) {
      ...
      if (!(signals & 0x1) && t0_is_high_) {
        ...
        if (t0_seen_ == 1) {
          EUDAQ_INFO("CLIDTD: Detected 1st T0 signal in event: " +
                     std::to_string(ev->GetEventNumber()) + " (ts signal)");
          // Discard this event:
          return false;
        } else {
          // throw exception and interrupt analysis:
          throw DataInvalid("CLICTD: Detected 2nd T0 signal in event: " +
      ...
      if ((triggers & 0x1) && !(signals & 0x1)) {
        if (time <= 10) {
          t0_seen_++;
          if (t0_seen_ == 1) {
            EUDAQ_INFO("CLICTD: Detected 1st T0 signal directly: T0 flag at " +
                       to_string(time) + "ns");
            // Discard this event:
            return false;
          } else {
            // throw exception and interrupt analysis:
            throw DataInvalid(
                "CLICTD: Detected 2nd T0 signal directly: T0 flag at " +
                to_string(time) + "ns");
          }
        } else {
          EUDAQ_INFO("CLIDTD: Detected T0 signal directly: T0 flag at " +
                     to_string(time) +
                     "ns. This did not reset the counter and was ignored in "
                     "the analysis.");
        }
      }
    }
  ...
  if (!full_shutter) {
    EUDAQ_WARN("Frame without shutter timestamps: " +
               std::to_string(ev->GetEventNumber()));
    return false;
  }
  ...
  if (shutter_open > shutter_close) {
    EUDAQ_WARN("Frame with shutter close before shutter open: " +
               std::to_string(ev->GetEventNumber()));
    t0_seen_++;
    if (t0_seen_ == 1) {
      EUDAQ_INFO("CLICTD: Detected 1st T0 signal indirectly: shutter_close "
                 "earlier than shutter_open in event " +
                 std::to_string(ev->GetEventNumber()) + " (ts jump)");
      return false;
    } else {
      // throw exception and interrupt analysis:
      throw DataInvalid("CLICTD: Detected 2nd T0 signal indirectly: "
                        "shutter_close earlier than shutter_open in event " +
                        std::to_string(ev->GetEventNumber()) + " (ts signal)");
    }
  }
  ...
  if (last_shutter_open_ > shutter_open) {
    ...
    if (t0_seen_ == 1) {
      EUDAQ_INFO("CLICTD: Detected 1st T0 signal indirectly: shutter_open (" +
                 to_string(shutter_open) +
                 "ns) earlier than previous shutter_open (" +
                 to_string(last_shutter_open_) + "ns), time difference: " +
                 to_string(last_shutter_open_ - shutter_open) + "ns");
      // Discard this event:
      return false;
    } else if (t0_seen_ > 1) {
      // throw exception and interrupt analysis:
      throw DataInvalid(
          "CLICTD: Detected 2nd T0 signal indirectly: shutter_open (" +
  ...
  for (const auto &px : data) {
    auto pixel = dynamic_cast<dut::CLICTDPixelReadout *>(px.second.get());
    ...
    try {
      ...
      // Check if we want to skip low ToT values. Only skip if we actually read
      // a ToT value
    ...
    if (counting) {
      ...
      // Check if we want to skip low ToA values. Only skip if we actually read
      // a ToA value and are not on counting mode
