...
bool H2MEvent2StdEventConverter::Converting(
  ...
  if (m_first_time) {
    if (conf && conf->Has("calibration_path_tot")) {
      ...
      EUDAQ_INFO(
          "No calibration file path for ToT; data will be uncalibrated.");
    }
    m_first_time = false;
  }
  ...
  for (const auto &pixel : frame) {
    ...
    if (!vtot.empty()) {
      ...
      charge = (sqrt(a * a * t * t + 2 * a * b * t + 4 * a * c -
                     2 * a * t * static_cast<float>(tot) + b * b -
                     2 * b * static_cast<float>(tot) +
                     static_cast<float>(tot * tot)) +
...
void H2MEvent2StdEventConverter::loadCalibration(
    std::string path, char delim, std::vector<std::vector<float>> &dat) const {
  std::ifstream f;
  f.open(path);
  dat.clear();
  ...
  while (!f.eof()) {
    ...
    if (line.size() > 0 && isdigit(line.at(0))) {
      ...
      if (row.size() != 4) { // Expected A, B, C, D parameters
        throw DataInvalid(
            "Unexpected number of parameters in calibration file. Expected 4 "
            "parameters (A, B, C, D), but found " +
  ...
  if (dat.size() != 64 * 16) {
    throw DataInvalid("Something went wrong. Found only " + to_string(i) +
                      " entries. Not enough for H2M.\n\t");
  }
