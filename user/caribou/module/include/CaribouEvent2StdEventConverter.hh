...
namespace eudaq {
  ...
  // litlle sturct to store all components of a waveform and make it more
  // structured in the converter
  struct waveform {
  ...
  private:
    void decodeChannel(const size_t adc, const std::vector<uint8_t> &data,
  ...
  public:
    ...
    static const uint32_t m_id_factory =
        eudaq::cstr2hash("CaribouDSO9254AEvent");
  ...
  private:
    ...
    read_data(peary::utils::pearyRawData &rawdata, int evt, uint64_t &block_position,
              int n_channels);
    // get the trigger number from the digital waveforns. This is tailored to
    // the AIDA TLU operating in the AIDA+trigerID mode
    ...
    static void savePlots(std::vector<std::vector<waveform>> &analog,
                          std::vector<waveform> &digital, int evt, int run);
  ...
  private:
    ...
    static std::array<std::array<double, 32>, 32>
    getPixelDelays(std::string config);
  ...
  private:
    ...
    void loadCalibration(std::string path, char delim,
                         std::vector<std::vector<float>> &dat) const;
  };
} // namespace eudaq
