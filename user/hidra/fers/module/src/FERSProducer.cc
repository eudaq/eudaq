...
// Janus_Config_t J_cfg;                                   // struct with all
// parameters
...
public:
  ...
  void
  checkEntries(const std::map<int, std::deque<SpectEvent_t>> &m_conn_evque);
  size_t splitStringToIntArray(const std::string &input, char delimiter,
                               int *result, size_t maxSize);
  int read_pedestal(const char *filename, int pid, uint16_t lgped[64],
                    uint16_t hgped[64]);
...
void FERSProducer::DoInitialise() {
  ...
  if (!fers_eth_address.empty()) {
    ...
    for (size_t iIP = 0; iIP < count; ++iIP) {
      ...
      if (ret == 0) {
        ...
        shmp->FERS_LastSrvEvent_us[fers_group]
                                  [shmp->connectedboards[fers_group]] =
            std::chrono::high_resolution_clock::now();

        EUDAQ_INFO(
            "check shared on board " +
            std::to_string(shmp->connectedboards[fers_group]) + ": " +
            std::string(tmp_path) + "*" +
            std::to_string(
                shmp->handle[fers_group][shmp->connectedboards[fers_group]]));
        m_conn_evque[shmp->connectedboards[fers_group]].clear();
        shmp->connectedboards[fers_group]++;
      } else {
        EUDAQ_THROW("Bords at " + std::string(tmp_path) + " error " +
                    std::to_string(ret));
      }
    }
  }
  ...
  if (fers_ip_address.find("tdl") != std::string::npos) {
    ...
    if (!FERS_IsOpen(cpath)) {
      ...
      ret = FERS_OpenDevice(
          cpath, &cnc_handle[cnc]); // open connection to the concetrator
      ...
      for (int i = 0; i < 8; i++) { // Loop over all the TDlink chains
        if (CncInfo.ChainInfo[i].BoardCount > 0) {
          EUDAQ_INFO("TDlink " + std::to_string(i) +
                     " Connected Boards Count " +
                     std::to_string(CncInfo.ChainInfo[i].BoardCount));
          // connection_path[strlen(connection_path) - 1] = '\0';
          int ROmode = ini->Get("FERS_RO_MODE", 0);
          int allocsize;
          char tmp_path[100];
          for (brd = 0; brd < CncInfo.ChainInfo[i].BoardCount;
               brd++) { // Loop over all the boards
            // std::sprintf(tmp_path,"%s%d", connection_path,brd);
            std::sprintf(tmp_path, "%s:tdl:%d:%d", c_ip.c_str(), i,
                         brd); // CNC IP : chain#: Brd#
            ret =
                FERS_OpenDevice(tmp_path, &handle); // open conection to a board
            if (ret == 0) {
              EUDAQ_INFO("Bords at " + std::string(tmp_path) +
                         " connected to handle " + std::to_string(handle));
              FERS_InitReadout(handle, ROmode, &allocsize);
              ...
              shmp->handle[fers_group][shmp->connectedboards[fers_group]] =
                  handle;
              shmp->FERS_TDLink[fers_group][shmp->connectedboards[fers_group]] =
                  1;
              shmp->FERS_LastSrvEvent_us[fers_group]
                                        [shmp->connectedboards[fers_group]] =
                  std::chrono::high_resolution_clock::now();

              EUDAQ_INFO("check shared on board " +
                         std::to_string(shmp->connectedboards[fers_group]) +
                         ": " + std::string(tmp_path));
              m_conn_evque[shmp->connectedboards[fers_group]].clear();
              shmp->connectedboards[fers_group]++;
  ...
  EUDAQ_WARN("FERS: # connected boards is " +
             std::to_string(shmp->connectedboards[fers_group]));
}
...
void FERSProducer::DoConfigure() {
  ...
  if (ret != 0)
    EUDAQ_THROW("Cannot load FERS configuration from file " +
                fers_conf_filename);
  ...
  for (int kbrd = 0; kbrd < shmp->connectedboards[fers_group];
       kbrd++) { // loop over boards

    // std::cout << "FERS_configure vhandle =
    // "<<shmp->handle[fers_group][kbrd]<<std::endl;
    ...
    if (!fers_ped_filename.empty() && !disable_ped) {
      ...
      ret =
          read_pedestal(fers_ped_filename.c_str(),
                        FERS_pid(shmp->handle[fers_group][kbrd]), LGped, HGped);
      ...
      FERS_ReadPedestalsFromFlash(shmp->handle[fers_group][kbrd], date,
                                  LGped_check, HGped_check, DCoffset);
      std::cout << "Ped FERS PID = " << FERS_pid(shmp->handle[fers_group][kbrd])
                << std::endl;
      for (int kk = 0; kk < 64; kk++)
        std::cout << kk << " LG = " << LGped[kk] << ", HG = " << HGped[kk]
                  << " LGread = " << LGped_check[kk]
                  << ", HGread = " << HGped_check[kk] << std::endl;
    }

    ret |= FERS_configure(shmp->handle[fers_group][kbrd], CFG_HARD);
    // std::cout<<"3333 - 3333 FERScfg[6]->HV_IndivAdj[52] =
    // "<<FERScfg[6]->HV_IndivAdj[52]<<std::endl;
    ...
    /*
                    // Pedestal calibration: check the factory ped data stored
       in flash

                    char date[20];
                    uint16_t pedLG[FERSLIB_MAX_NCH], pedHG[FERSLIB_MAX_NCH];
                    uint16_t DCoffset[4];
                    FERS_ReadPedestalsFromFlash(shmp->handle[fers_group][brd],
       date, pedLG, pedHG, DCoffset); std::cout <<"FERS PID =
       "<<FERS_pid(shmp->handle[fers_group][brd])<<std::endl; std::cout <<"CH
       PedLG   PedHG        CH   PedLG   PedHG"<<std::endl; for(int ch=0; ch<32;
       ch++) { std::cout <<ch<<", "<<  pedLG[ch]<<", "<<  pedHG[ch]<<", "<<
       32+ch<<", "<<  pedLG[32+ch]<<", "<<  pedHG[32+ch]<<std::endl;
                    }
                    std::cout <<"DCoffset: "<<DCoffset[0]<<", "<<
       DCoffset[1]<<", "<<DCoffset[2]<<", "<<DCoffset[3]<<std::endl;
    */

    ret_dummy = FERS_HV_Get_Imax(shmp->handle[fers_group][kbrd],
                                 &fers_dummyvar); // read back from fers
    if (ret == 0) {
    ...
    ret_dummy = FERS_HV_Get_Vbias(shmp->handle[fers_group][kbrd],
                                  &fers_dummyvar); // read back from fers
    if (ret == 0) {
      ...
      EUDAQ_WARN(
          "check shared in board " + std::to_string(kbrd) +
...
void FERSProducer::DoStartRun() {
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock> tp_start_aq =
      std::chrono::high_resolution_clock::now();
  shmp->FERS_Aqu_start_time_us = tp_start_aq;

  // here the hardware is told to startup
  for (brd = 0; brd < shmp->connectedboards[fers_group];
       brd++) { // loop over boards
    ...
    // std::chrono::duration_cast<std::chrono::microseconds>(return_sq -
    // tp_start_aq);
  }
  ...
  if (!sw_trigger) {
    for (int brd = 0; brd < shmp->connectedboards[fers_group]; brd++) {
      ...
      // FERS_WriteRegister(shmp->handle[fers_group][brd], a_t0_out_mask, 32);
      // // Sends BUSY status
    }
  ...
  /*
                          if (streq(value, "T1-IN")) FERScfg[brd]->T1_outMask =
     0x001; else if (streq(value, "BUNCHTRG")) FERScfg[brd]->T1_outMask = 0x002;
                          else if (streq(value, "Q-OR"))
     FERScfg[brd]->T1_outMask = 0x004; else if (streq(value, "RUN"))
     FERScfg[brd]->T1_outMask = 0x008; else if (streq(value, "PTRG"))
     FERScfg[brd]->T1_outMask = 0x010; else if (streq(value, "BUSY"))
     FERScfg[brd]->T1_outMask = 0x020; else if (streq(value, "DPROBE"))
     FERScfg[brd]->T1_outMask = 0x040; else if (streq(value, "TLOGIC"))
     FERScfg[brd]->T1_outMask = 0x080; else if (streq(value, "SQ_WAVE"))
     FERScfg[brd]->T1_outMask = 0x100; else if (streq(value, "TDL_SYNC"))
     FERScfg[brd]->T1_outMask = 0x200; else if (streq(value, "RUN_SYNC"))
     FERScfg[brd]->T1_outMask = 0x400; else if (streq(value, "ZERO"))
     FERScfg[brd]->T1_outMask = 0x000;
  */
}
...
void FERSProducer::RunLoop() {
  ...
  // std::chrono::time_point<std::chrono::high_resolution_clock> runloop_time =
  // std::chrono::high_resolution_clock::now(); auto run_time =
  // std::chrono::duration_cast<std::chrono::microseconds>(
  // std::chrono::high_resolution_clock::now() - runloop_time);
  ...
  int newData = 0;
  auto time_diff = std::chrono::system_clock::now() -
                   shmp->FERS_LastSrvEvent_us[fers_group][0];
  auto time_diff_sec =
  ...
  if (time_diff_sec.count() > 2) {
    for (int ibrd = 0; ibrd < shmp->connectedboards[fers_group]; ibrd++) {
      ...
      ret |= FERS_HV_Get_DetectorTemp(shmp->handle[fers_group][ibrd],
                                      &tempDetector);
  ...
  while (!m_exit_of_run) {
    ...
    auto elapsedFERS = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - shmp->FERS_last_event_time_us);
    auto elapsedDRS = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - shmp->DRS_last_event_time_us);

    if (spill_detect && elapsedFERS.count() > 2500 &&
        elapsedFERS.count() < 3000 && elapsedDRS.count() > 2000) {
      // std::cout<<"---3333---  End od spill" <<std::endl;

      int ret = check_TRIG_alignment(); // All trigger IDs should match here.
      if (ret < 0) {
        for (int cbrd = 0; cbrd < shmp->connectedboards[fers_group];
             cbrd++) { // loop over boards
    ...
    if (read_boards) {
      for (int ibrd = 0; ibrd < shmp->connectedboards[fers_group]; ibrd++) {
        ...
        // std::chrono::high_resolution_clock::now() - runloop_time);
        // std::cout<<"---3333--- time in ms before FERS_GetEvent "
        // <<run_time.count()/1000.<<std::endl;
        ...
        if (shmp->FERS_TDLink[fers_group][ibrd]) {
          status = FERS_GetEvent(shmp->handle[fers_group], &iibrd,
                                 &DataQualifier, &tstamp_us, &Event, &nb);
        } else {
          status =
              FERS_GetEventFromBoard(shmp->handle[fers_group][ibrd],
                                     &DataQualifier, &tstamp_us, &Event, &nb);
        }

        if (status < 0) {
          EUDAQ_THROW(
              "FERS: Readout failure,  ret = " + std::to_string(status) +
              " board = " + std::to_string(iibrd));
        }
        ...
        if (nb > 0 && DataQualifier == 17) { // Data event in Spec
          ...
          if (EventSpect->trigger_id >
              shmp->FERS_last_trigID[fers_group][iibrd]) {
            shmp->FERS_last_event_time_us =
                std::chrono::high_resolution_clock::now();
          ...
          std::cout << "---3333--- board="
                    ...
                    << " tstamp_us = " << std::fixed << std::setprecision(3)
                    << EventSpect->tstamp_us
                    //	<<" nb= "<<nb
                    << std::endl;
        }
        ...
        // run_time =  std::chrono::duration_cast<std::chrono::microseconds>(
        // std::chrono::high_resolution_clock::now() - runloop_time);
        // std::cout<<"---3333--- time in ms after FERS_GetEvent "
        // <<run_time.count()/1000.<<std::endl;
      }
    } else {
      DataQualifier = 1000;
      while (newData < shmp->connectedboards[fers_group] &&
             DataQualifier > 0) { // read all data from the boards
        status = FERS_GetEvent(shmp->handle[fers_group], &bindex,
                               &DataQualifier, &tstamp_us, &Event, &nb);
        if (status < 0) {
          EUDAQ_THROW("FERS: Readout failure,  ret = " +
        ...
        if (nb > 0 && DataQualifier == 17) { // Data event in Spec
          ...
          std::cout << "---3333---  status=" << status << " board=" << bindex
                    << " DataQualifier= " << DataQualifier << " trigger_id = "
                    << EventSpect->trigger_id
    ...
    if (no_trigg > 0)
      checkEntries(m_conn_evque); // detect no data sent by FERS board(s)

    if (newData >=
        shmp->connectedboards[fers_group]) { // if evt*boards >= boards, i.e. at
                                             // least one complete event
                                             // candidate
      ...
      for (int ievt = 0; ievt < Nevt; ievt++) {
        ...
        trigger_n = -1;
        for (auto &conn_evque :
             m_conn_evque) { // find the min trigger cnt in the queue
        ...
        if (bCntr !=
            shmp->connectedboards[fers_group]) { // check for missing data from
                                                 // some of the FERS boards
          ...
          // EUDAQ_WARN("FERS: Event alignment failed with
          // "+std::to_string(m_conn_ev.size())
          //	+" board's records instead of
          ...
          for (int brd = 0; brd < shmp->connectedboards[fers_group]; brd++) {
            if (m_flag_ts && brd == 0) {
              auto du_ts_beg_us =
                  std::chrono::duration_cast<std::chrono::microseconds>(
                      shmp->FERS_Aqu_start_time_us - get_midnight_today());
              auto tp_trigger0 = std::chrono::microseconds(
                  static_cast<long int>(m_conn_ev[brd].tstamp_us));
              du_ts_beg_us += tp_trigger0;
              std::chrono::microseconds du_ts_end_us(du_ts_beg_us +
                                                     m_us_evt_length);
              ev->SetTimestamp(static_cast<uint64_t>(du_ts_beg_us.count()),
                               static_cast<uint64_t>(du_ts_end_us.count()));
            }

            std::vector<uint8_t> data;
            make_headerFERS(shmp->flat_idx[fers_group][brd],
                            FERS_pid(shmp->handle[fers_group][brd]),
                            shmp->hv_Vmon[fers_group][brd],
                            shmp->hv_Imon[fers_group][brd],
                            shmp->tempDet[fers_group][brd],
                            shmp->tempFPGA[fers_group][brd], &data);
            // Add data here
...
void FERSProducer::checkEntries(
    const std::map<int, std::deque<SpectEvent_t>> &m_conn_evque) {
  size_t max_size = 0;
...
size_t FERSProducer::splitStringToIntArray(const std::string &input,
                                           char delimiter, int *result,
                                           size_t maxSize) {
  ...
  while (std::getline(ss, item, delimiter)) {
    if (index < maxSize) {
      try {
        result[index++] =
            std::stoi(item); // Convert to int and store in the array
      } catch (const std::invalid_argument &e) {
        std::cerr << "Invalid input: " << item << " is not an integer."
                  << std::endl;
      }
    } else {
...
int FERSProducer::read_pedestal(const char *filename, int pid,
                                uint16_t lgped[64], uint16_t hgped[64]) {
