...
{
//    const int diffParam = 100;
    const int diffParam = 50;
	...
	//	gettimeofday(&then, NULL);
		for (unsigned int acluster = 0; acluster < aClusters.size();acluster++)
		{
			...
			for (unsigned int bcluster = 0; bcluster < bClusters.size();bcluster++)
			{
				...
				else if(checkCorrelations(oneAcluster, oneBcluster))
				{
...
//                }
            ...
            {
                for (unsigned int nextPlaneIndex = ++currPlaneIndex; nextPlaneIndex < nPlanes; ++nextPlaneIndex)
                {
...
//                                && checkCorrelations(singleTrack.back().second, currentCluster) )
                        if( checkCorrelations(singleTrack.back().second, currentCluster) )
                        {
