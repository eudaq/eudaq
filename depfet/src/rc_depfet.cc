...
int TCP_CONNECT(char *HOST, int PORT) {
    ...
    if ((ret=send(Socket, (void *)INFO, strlen(INFO), 0))!=strlen(INFO)) {
	printf("Error Send INFO ret=%d of (%d)\n",ret,strlen(INFO)); perror("send to RC");
    }    
    return Socket;
}
//------------------------------------------------------------------------------------
...
//    char command[256];
    ...
    if ((ret=send(cmd_socket, (void *)command.c_str(),command.length(), 0))!=command.length()) {
	printf("Error Send INFO ret=%d of (%d)\n",ret,command.length()); perror("send to RC");
    }    
