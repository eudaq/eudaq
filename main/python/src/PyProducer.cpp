...
class PyProducer : public eudaq::Producer {
  public:
    ...
    virtual void OnConfigure(const eudaq::Configuration & param) {
      ...
      SetStatus(eudaq::Status::LVL_BUSY, "Waiting for Python-side configure", eudaq::Status::ST_UNCONF);
	...
	SetStatus(eudaq::Status::LVL_OK, "Configured (" + m_config->Name() + ")", eudaq::Status::ST_CONF);
      }
    }
	...
	SetStatus(eudaq::Status::LVL_ERROR, "Cannot start run/unconfigured!", eudaq::Status::ST_ERROR);
	m_internalstate = Error;
      } else {
	m_internalstate = StartingRun;
	SetStatus(eudaq::Status::LVL_BUSY, "Waiting for Python-side run start" ,eudaq::Status::ST_CONF);
      }
    ...
    virtual void OnStopRun() {
      ...
      SetStatus(eudaq::Status::LVL_BUSY, "Waiting for Python-side run stop", eudaq::Status::ST_RUNNING);
