...
namespace eudaq {
  ...
  void FileWriterFactory::do_register(const std::string &name,
                                      FileWriterFactory::factoryfunc func) {
    // std::cout << "DEBUG: Registering FileWriter: " << name << std::endl;
    FileWriterMap()[name] = func;
  }

  FileWriter *FileWriterFactory::Create(const std::string &name,
                                        const std::string &params) {
    map_t::const_iterator it =
        FileWriterMap().find(name == "" ? "native" : name);
    if (it == FileWriterMap().end())
