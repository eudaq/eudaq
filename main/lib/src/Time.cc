...
namespace eudaq {
  ...
  Time Time::Current() {
    ...
    if (gettimeofday(&tv, 0)) {
      EUDAQ_THROW(std::string("Error getting current time: ") +
                  strerror(errno));
    }
    return tv;
  }
  ...
  Time::Time(int year, int month, int date, int hour, int minute, int sec,
             int usec) {
    struct tm time;
    // time.tm_gmtoff = 0;
    time.tm_isdst = -1;
  ...
  std::string Time::Formatted(const std::string &format) const {
    ...
    while ((i = fmt.find('%', pos)) != std::string::npos) {
      ...
      if (c >= 1 && c <= 6) {
        ...
        fmt = std::string(fmt, 0, i) + std::string(usecs, 0, c) +
              std::string(fmt, i + 2);
        // std::cout << "fmt=" << fmt << std::endl;
        pos += c - 2;
        // std::cout << "pos=" << pos << std::endl;
