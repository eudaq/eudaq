...
namespace eudaq {
  ...
  void EventnumberSyncDataCollector::DoConnect(std::shared_ptr<const ConnectionInfo> id){
    std::unique_lock<std::mutex> lk(m_mtx_map);
  ...
  void EventnumberSyncDataCollector::DoDisconnect(std::shared_ptr<const ConnectionInfo> id){
    std::unique_lock<std::mutex> lk(m_mtx_map);
    std::string pdc_name = id->GetName();
    if(m_que_event.find(pdc_name) == m_que_event.end())
  ...
  void EventnumberSyncDataCollector::DoReceive(std::shared_ptr<const ConnectionInfo> id, EventUP ev){
    std::unique_lock<std::mutex> lk(m_mtx_map);
    std::string pdc_name = id->GetName();
