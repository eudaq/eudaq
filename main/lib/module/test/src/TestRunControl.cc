...
public:
  ...
  void OnReceive(const eudaq::ConnectionInfo & id, std::shared_ptr<eudaq::Status> status) override final;
  ...
  static const uint32_t m_id_factory = eudaq::cstr2hash("TestRunControl");
};

namespace{
  auto dummy0 = eudaq::Factory<eudaq::RunControl>::
    Register<TestRunControl, const std::string&>(TestRunControl::m_id_factory);
}



TestRunControl::TestRunControl(const std::string & listenaddress)
      : eudaq::RunControl(listenaddress){

}

void TestRunControl::OnReceive(const eudaq::ConnectionInfo & id, std::shared_ptr<eudaq::Status> status) {
  std::cout << "Receive:    " << *status << " from " << id << std::endl;
}

void TestRunControl::OnConnect(const eudaq::ConnectionInfo & id) {
  std::cout << "Connect:    " << id << std::endl;
}

void TestRunControl::OnDisconnect(const eudaq::ConnectionInfo & id) {
  std::cout << "Disconnect: " << id << std::endl;
}


void TestRunControl::Exec(){
  bool done = false;
  bool help = true;
  do {
    if (help) {
