  ...
  public:
    ...
    virtual unsigned GetTriggerID(const Event &ev) const {
      ...
      if (const RawDataEvent *rev = dynamic_cast<const RawDataEvent *>(&ev)) {
        // TLU ID is contained in the trailer of the last data frame in the 4th
        // and
    ...
    virtual bool GetStandardSubEvent(StandardEvent &sev,
      ...
      if (data.size() - 4 != unpack_fh(it, EVENT_LENGTH)) {
        ...
        cout << "\tEvent Length not correct!" << endl
             << "\tEither order not correct or Event corrupt!!" << endl
             << "\tExpected Length: " << EVENT_LENGTH
             << " Seen: " << data.size() - 4 << endl;
        return false;
      }
      unpack_fh(it, FRAME_LENGTH);
      if (FRAME_LENGTH == 9) { // discard events with EoE-Frame as first frame,
                               // as they are empty or corrupted
        printf(
      ...
      framepos[(int)*it] =
          make_pair(it + 1, FRAME_LENGTH - 12); // id and position of this frame
      it++;
      ...
      while (it != data.end()) {
        if (unpack_fh(it, FRAME_LENGTH) == 9) { // EoE-Frame
          ...
          if (unpack_fn(it, evn) != EVENT_NUM) { // EvNum exception
            cout << "Event Number ERROR: " << endl
                 << "\tEventnumber mismatch!" << endl
                 << "\tDropping Event!" << endl;
            sev.SetFlags(Event::FLAG_BROKEN);
            return false;
          }
          if (unpack_b(it, ds, 3) != DS) {
            cout << "Data Specifier ERROR:\n"
                 << "\tData specifier do not fit!" << endl
                 << "\tDropping Event!" << endl;
            sev.SetFlags(Event::FLAG_BROKEN);
            return false;
          }
          ...
          if ((int)*it <= 0 || (int)*it > 7) {
            cout << "Frame Counter ERROR:\n"
                 << "\tFramecount not possible! Range: 0..7 + EoE-frame" << endl
                 << "\tDropping Event" << endl;
            sev.SetFlags(Event::FLAG_BROKEN);
            return false;
          }
      ...
      unsigned int nhit = ((EVENT_LENGTH)-nframe * (12 + 4) - 24 - (9 + 4)) /
                          2; // estimate the number of hits
      ...
      for (unsigned int ihit = 0; ihit < nhit; ++ihit) {
        ...
        if (buffer & 0x8000) {
          ...
          // cout << "good 0x" << hex << (unsigned int)buffer << dec << '\t' <<
          // col << '\t' << row << endl;
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent &lev,
      ...
      try {
        pALPIDEssDataCollection = static_cast<LCCollectionVec *>(
            lev.getCollection("zsdata_pALPIDEss"));
        pALPIDEssDataCollectionExists = true;
      } catch (lcio::DataNotAvailableException &) {
        pALPIDEssDataCollection =
            new lcio::LCCollectionVec(lcio::LCIO::TRACKERDATA);
        pALPIDEssDataCollectionExists = false;
      }

      CellIDEncoder<lcio::TrackerDataImpl> pALPIDEssDataEncoder(
          eutelescope::EUTELESCOPE::ZSDATADEFAULTENCODING,
          pALPIDEssDataCollection);
      lcio::TrackerDataImpl *pALPIDEssFrame = new lcio::TrackerDataImpl();
      pALPIDEssDataEncoder["sensorID"] = m_PalpidessPlaneNo;
      pALPIDEssDataEncoder["sparsePixelType"] =
          eutelescope::kEUTelSimpleSparsePixel;
      ...
      if (pALPIDEssDataCollection->size() != 0)
        lev.addCollection(pALPIDEssDataCollection, "zsdata_pALPIDEss");
      else if (!pALPIDEssDataCollectionExists) {
        delete pALPIDEssDataCollection;
        pALPIDEssDataCollection = 0x0;
      }
  ...
  private:
    ...
    PALPIDESSConverterPlugin()
        : DataConverterPlugin(EVENT_TYPE), m_PalpidessPlaneNo(0), m_Vbb(),
          m_Vrst(), m_Vcasn(), m_Vcasp(), m_Ithr(), m_Vlight(), m_AcqTime(),
          m_TrigDelay() {}
