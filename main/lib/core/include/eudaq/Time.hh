...
#define TIME_DEFAULT_FORMAT                                                    \
  "%Y-%m-%d %H:%M:%S.%3" // I thing it is impossible to export static variables
                         // from a dll. so i made it to a macro this works fine.
  ...
  public:
    ...
    Time(int year, int month, int date, int hour = 0, int minute = 0,
    ...
    operator const timeval() const { return GetTimeval(); }
    std::string
    Formatted(const std::string &format = TIME_DEFAULT_FORMAT) const;
    static Time Current();
  ...
  inline std::ostream &operator<<(std::ostream & os, const timeval &tv) {
    ...
    return os << tv.tv_sec << '.' << std::setw(6) << std::setfill('0')
