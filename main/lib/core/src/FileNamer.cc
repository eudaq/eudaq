...
namespace eudaq {
  ...
  const std::string FileNamer::default_pattern = "run$6R$S$3N$X";
  ...
  FileNamer::FileNamer(const std::string &p) {
    ...
    for (;;) {
      ...
      if (i1 >= pattern.length())
        EUDAQ_THROW("Bad file pattern: '" + pattern + "'");
      if (pattern[i1] == '-' || pattern[i1] == '+')
        i1 += 1;
      ...
      if (i2 != i1) {
        num = from_string(
            pattern.substr(i1 - (pattern[i1 - 1] == '-'), i2 - i1), 0);
      }
      m_parts.push_back(part_t(pattern[i2], num));
      // std::cout << "part " << m_parts.back().name << ", " <<
      // m_parts.back().len << std::endl;
      i0 = i2 + 1;
      if (i0 == pattern.length())
        break;
    }
  }
