...
// The Idea is that the user can define a condition that need to be true to
// define if an event is sync or not
...
namespace eudaq {
  ...
  public:
    ...
    int AddDetectorElementToProducerQueue(
        int fileIndex, std::shared_ptr<eudaq::DetectorEvent> dev);
    ...
    virtual bool
    compareTLUwithEventQueue(std::shared_ptr<eudaq::Event> &tlu_event,
                             SyncBase::eventqueue_t &event_queue);
    virtual bool
    compareTLUwithEventQueues(std::shared_ptr<eudaq::Event> &tlu_event);
