...
namespace eudaq {
  ...
  public:
    OptionParser(const std::string &name, const std::string &version,
                 const std::string &desc = "", int minargs = -1,
                 int maxargs = -1)
        : m_name(name), m_ver(version), m_desc(desc), m_minargs(minargs),
          m_maxargs(maxargs) {}
    OptionParser &Parse(const char **args);
    OptionParser &Parse(char **args) {
      return Parse(const_cast<const char **>(args));
    }
    ...
    int HandleMainException(std::ostream &err = std::cerr,
  ...
  public:
    OptionBase(OptionParser &p, const std::string &shortname,
               const std::string &longname, const std::string &deflt = "",
               const std::string &argname = "", const std::string &desc = "")
        : m_argname(argname), m_deflt(deflt), m_desc(desc),
          m_hasarg(argname != ""), m_isset(false) {
      if (shortname != "")
        m_names.push_back("-" + shortname);
      if (longname != "")
        m_names.push_back("--" + longname);
      p.AddOption(this);
    }
    virtual ~OptionBase() {}
  ...
  public:
    Option(OptionParser &p, const std::string &shortname,
        ...
        : OptionBase(p, shortname, longname, "", argname, desc),
          m_sep(sep.length() ? sep : ",") {}
    const std::vector<T> &Value() const { return m_value; }
    void SetValue(const std::vector<T> &val = std::vector<T>()) {
      m_value = val;
    }
  ...
  private:
    virtual void DoParsing(const std::string & /*name*/,
      ...
      do {
        ...
        // std::cout << '"' << arg << '"' << ".find(" << m_sep << ", " << start
        // << ") = " << i << std::endl;
        std::string valstr(arg, start,
                           (i == std::string::npos ? arg.length() : i) - start);
