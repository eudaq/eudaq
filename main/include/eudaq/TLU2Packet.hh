...
namespace eudaq {
  ...
  public:
    ...
    *
    ...
    inline uint64_t GetPacketSubType() const { return
    m_header.data.packetSubType; };
    inline void SetPacketSubType( uint64_t type ) { m_header.data.packetSubType
    = type; };
    */
    ...
    DECLARE_ENUM_CLASS(TLU2MetaDataType, RUN_NUMBER, TRIGGER_COUNTER,
                       TRIGGER_TIMESTAMP);
...
static inline int GetType( uint64_t meta_data ) { return getBits(ENTRY_TYPE,
meta_data ); };
static inline void SetType( uint64_t& meta_data, int type ) {
setBits(ENTRY_TYPE,  meta_data, type ); };
static inline bool IsTLUBitSet( uint64_t meta_data ) { return getBits(TLU,
meta_data ); };
static inline uint64_t GetCounter( uint64_t meta_data ) { return
getBits(COUNTER, meta_data ); };
static inline void SetCounter( uint64_t& meta_data, uint64_t data ) {
