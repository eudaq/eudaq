...
public:
    ...
    static Mupix4DataProxy from_event(const RawDataEvent & e)
    {
        ...
        if (block.size() !=  proxy.size()) {
            EUDAQ_WARN("event " + to_string(e.GetEventNumber())
                       ...
                       + "received " + to_string(block.size()) + " "
                       + "from header " + to_string(proxy.size()) + " ");
            return Mupix4DataProxy();
        }
    ...
    uint32_t event_id() const { return eudaq::getlittleendian<uint32_t>(&_data[0]); }
    uint16_t trigger_id() const { return eudaq::getlittleendian<uint16_t>(&_data[4]); }
    uint8_t  trigger_graystamp() const { return _data[6]; }
    uint8_t  trigger_timestamp() const { return gray_decode(trigger_graystamp()); }
    ...
    uint8_t hit_timestamp(unsigned i) const { return gray_decode(hit_graystamp(i)); }
