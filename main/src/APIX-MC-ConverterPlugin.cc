...
namespace eudaq {
  ...
  StandardPlane APIXMCConverterPlugin::ConvertPlane(const std::vector<unsigned char> & data, unsigned id, int FE) const {
    ...
    plane.SetSizeZS(18, 160, npixels, 16, StandardPlane::FLAG_ACCUMULATE);

    unsigned int subtrigger = 1;
    int j = 0;

    for (size_t i=0;i<data.size();i=i+4) {
      unsigned int one_line = getlittleendian<unsigned int>(&data[i]);
        ...
        if ((one_line & 0x02000000)>>25 != 0x1) {
          ...
          if (chip == FE) { // otherwise the data belongs to a different frame
            ...
            */
            ...
            std::cout << "SetPixel " << j << ", " << xpos << ", " << ypos << ", " << tot << ", " << subtrigger << std::endl;

            plane.SetPixel(j, xpos, ypos, tot, subtrigger);
            j++;
          }
        }
      }
