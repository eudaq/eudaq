...
template <typename T> inline void pack(vector<unsigned char> &dst, T &data) {
...
public:
  ...
  PalpidessProducer(const string &name, const string &runcontrol)
      : eudaq::Producer(name, runcontrol), m_run(0), m_ev(0), stopping(false),
        running(false), done(false), request_sent(false) {
  ...
  virtual void OnConfigure(const eudaq::Configuration &param) {
    ...
    for (int i_param = 1; i_param < n_param; ++i_param) {
      ...
      options[i_param].second =
          param.Get(options[i_param].first, param_default[i_param]);
    ...
    printf("\n--------------------- Start Servers --------------------\n");
    if (fPalpidess->get_SD() < 0)
      fPalpidess->create_server_udp();
    else
      printf("Server 0 for FEC HLVDS has already been up\n");
  ...
  ///////////////////////////////////////////////////////////////////////////////
  // This gets called when the Run Control is terminating and we should also
  // exit
  ///////////////////////////////////////////////////////////////////////////////
      ...
      if (!running && !stopping) { // called while run not started yet
        ...
        if (!stopping) {
            ...
            if (!evt_bad) {
              ...
              if (evt_status == 0 && max_frame_id == -1)
                ...
                cout << "** found wrong event id "
                     << extract_event_id(data_packet) << " instead of "
                     << evt_id << " in the LVDS event " << endl;
              }
              // handle the frame id
              if (num_byte_rcv == 9) {
          ...
          if (evt_finished) {
            ...
            bufferEvent_LVDS.insert(bufferEvent_LVDS.begin(), evt_size.begin(),
                                    evt_size.end());
            // add event check
            // TODO TODO
          } else {
            cout << "** LVDS event not finished" << endl
                 ...
                 << "** event status:     0x" << hex << evt_status << dec
                 << endl;
          }
...
private:
  ...
  bool check_socket(int fd, float timeout) // timeout in s
  {
    ...
    struct timeval tv_timeout = {(int)timeout,
                                 (int)(timeout * 1000000.) % 1000000};

    int select_retval =
        select(fd + 1, &fdset, NULL, NULL, (timeout < 0) ? NULL : &tv_timeout);
...
int main(int /*argc*/, const char **argv) {
  ...
  eudaq::Option<string> rctrl(op, "r", "runcontrol", "tcp://localhost:44000",
                              "address", "The address of the RunControl.");
  eudaq::Option<string> level(
      op, "l", "log-level", "NONE", "level",
      "The minimum level for displaying log messages locally");
  eudaq::Option<string> name(op, "n", "", "pALPIDEss", "string",
...
string const PalpidessProducer::param_str[] = {
    "PalpidessPlaneNo", "Vrst",    "Vcasn", "Vcasp",    "Ithr",
    "Vlight",           "AcqTime", "Vbb",   "TrigDelay"}; // crosscheck n_param!
string const PalpidessProducer::param_default[] = {
    "0",   "1.6",  "0.4", "0.6", "2.05",
    "0.0", "1.54", "0.0", "0.0"}; // crosscheck n_param!
