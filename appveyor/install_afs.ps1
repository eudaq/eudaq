function main () {
	...
	Start-Process "C:\\Heimdal-AMD64-full-7-4-0-40.msi" -ArgumentList "/quiet /qn /norestart /log install_heimdal.log ADDLOCAL=ALL" -Wait;
	Start-Process msiexec -ArgumentList "/i C:\\openafs-en_US-64bit-1-7-3100.msi /quiet /qn /norestart /log install_openafs.log ADDLOCAL=ALL InstallMode=Typical AFSCELLNAME=desy.de IAgree=yes" -Wait -verb runas;
	Start-Process "C:\\openafs-32bit-tools-en_US-1-7-3100.msi" -ArgumentList "/quiet /qn /norestart /log install_openafs_tools.log ADDLOCAL=ALL" -Wait;
    }  Else {
