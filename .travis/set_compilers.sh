...
if [[ $TRAVIS_OS_NAME == 'osx' ]]; then
	...
	if [[ $COMPILER == 'gcc' ]]; then
		...
		if [[ "${LLVM_VERSION}" != "" ]]; then
			...
			sudo apt-get update
			sudo apt-get install g++-5 gcc-5 clang-$LLVM_VERSION clang-$LLVM_VERSION-doc libclang-common-$LLVM_VERSION-dev libclang-$LLVM_VERSION-dev libclang1-$LLVM_VERSION libclang1-$LLVM_VERSION-dbg libllvm-$LLVM_VERSION-ocaml-dev libllvm$LLVM_VERSION libllvm$LLVM_VERSION-dbg liblldb-$LLVM_VERSION llvm-$LLVM_VERSION llvm-$LLVM_VERSION-dev llvm-$LLVM_VERSION-doc llvm-$LLVM_VERSION-examples llvm-$LLVM_VERSION-runtime clang-format-$LLVM_VERSION python-clang-$LLVM_VERSION lldb-$LLVM_VERSION-dev lld-$LLVM_VERSION libfuzzer-$LLVM_VERSION-dev  -y
