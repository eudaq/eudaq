...
if [[ $TRAVIS_OS_NAME == 'osx' ]]; then
	...
	if [[ $COMPILER == 'gcc' ]]; then
		...
		if [[ "${LLVM_VERSION}" == "6.0" ]]; then
			...
			sudo apt-get install g++-6 gcc-6 clang-6.0 clang-tools-6.0 clang-6.0-doc libclang-common-6.0-dev libclang-6.0-dev libclang1-6.0 libllvm6.0 liblldb-6.0-dev llvm-6.0 llvm-6.0-dev llvm-6.0-doc llvm-6.0-examples llvm-6.0-runtime clang-format-6.0 python-clang-6.0 lld-6.0 libfuzzer-6.0-dev  -y
