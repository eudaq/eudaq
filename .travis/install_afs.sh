...
./timeout3 "if [[ -d "/afs/desy.de/group/telescopes" ]]; then echo "Afs seems to work properly" elif [[ -d "/afs/cern.ch" ]]; then echo "Afs seems to work properly, but desy afs down?" else echo "Something wrong with the afs installation"	 fi"
