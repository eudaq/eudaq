    ...
    if (parts.size() == 1) {
      ...
      if (parts.size() == 7 && parts[0] == "***" && parts[3] == "at" &&
        ...
        eudaq::Time time(
            ...
            from_string(timeparts[4], 0), from_string(timeparts[5], 0),
            from_string(timeparts[6], 0) * 1000);
        return eudaq::LogMessage("*** " + parts[2] + " ***",
                                 eudaq::Status::LVL_USER,
                                 time).SetSender(parts[1]);
      }
...
QString LogMessage::operator[](int i) const {
  switch (i) {
  case 0:
    return m_createtime.Formatted("%H:%M:%S.%3").c_str();
  case 1:
    return m_time.Formatted("%H:%M:%S.%3").c_str();
  case 3:
    return firstline(this->Text(i)).c_str();
  case 5:
    return simple_file(this->Text(i)).c_str();
  case 6:
    return simple_func(this->Text(i)).c_str();
  default:
...
std::string LogMessage::Text(int i) const {
  switch (i) {
  case 0:
    return m_createtime.Formatted("%Y-%m-%d %H:%M:%S.%6");
  case 1:
  ...
  case 4:
    return GetSender();
  case 5:
    return m_file + (m_line == 0 ? "" : (":" + to_string(m_line)));
  case 6:
    return m_func;
  default:
    return "";
...
LogCollectorModel::LogCollectorModel(QObject *parent)
    : QAbstractListModel(parent), m_displaylevel(0), m_sorter(&m_all) {}

std::vector<std::string>
LogCollectorModel::LoadFile(const std::string &filename) {
...
bool LogCollectorModel::IsDisplayed(size_t index) {
  ...
  return (msg.GetLevel() >= m_displaylevel &&
          (m_displaytype == "" || m_displaytype == "All" ||
           msg.GetSenderType() == m_displaytype) &&
          (m_displayname == "" || m_displayname == "*" ||
           msg.GetSenderName() == m_displayname) &&
          m_search.Match(msg));
}
...
QModelIndex LogCollectorModel::AddMessage(const LogMessage &msg) {
  ...
  if (IsDisplayed(m_all.size() - 1)) {
    std::vector<size_t>::iterator it = std::lower_bound(
        m_disp.begin(), m_disp.end(), m_all.size() - 1, m_sorter);
